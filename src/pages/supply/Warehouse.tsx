...
export default function Warehouse() {
      ...
      const htmlContent = `
        ...
        <body>
          ...
          <div class="order-info">
            ...
            ${
              selectedItem?.image 
                ? `<img src="${selectedItem.image.startsWith('http') ? selectedItem.image : window.location.origin + '/image/' + selectedItem.image}" alt="فاتورة الشراء" class="invoice-image"/>`
                : ''
            }
          </div>
          <div class="order-info">
  ...
  const validateAddForm = () => {
    ...
    // Image optional: but if present check it is file type image
    if (addFormData.image && !addFormData.image.type.startsWith('image/')) {
      errors.image = 'الملف يجب أن يكون صورة';
    }
  ...
  const validateEditForm = () => {
    ...
    // Image optional: but if present check it is file type image
    if (editFormData.image && !editFormData.image.type.startsWith('image/')) {
      errors.image = 'الملف يجب أن يكون صورة';
    }
  ...
  // For Add Form Inputs including image file with preview
  const handleAddInputChange = (field: string, value: string | File | null) => {
    setAddFormData(prev => {
      ...
      if (field === 'receivedQty' || field === 'issuedQty') {
        updated.availableQty = calculateAvailableQty(
          field === 'receivedQty' ? (value as string) : prev.receivedQty,
          field === 'issuedQty' ? (value as string) : prev.issuedQty
        ).toString();
      }
    ...
    // If image file selected, set preview
  ...
  // For Edit Form Inputs including image file with preview
  ...
  // For add submit create FormData to send image
    ...
    try {
      ...
      for (const key in addFormData) {
        if (key === 'image') {
          ...
          // @ts-ignore
          formDataToSend.append(key, addFormData[key]);
        }
      }

      const response = await warehouseApi.addInventoryItem(formDataToSend);
        ...
        try {
          ...
          if (inventoryResponse.success && inventoryResponse.data) {
            ...
            const newItem = {
              ...
              ...addFormData,
              image: addFormData.image ? addFormData.image.name : null,
  ...
  // Edit submit - also FormData for image upload
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await warehouseApi.updateInventoryItem(selectedItem.id, formDataToSend);
  ...
  const handleEditClick = (item: any) => {
    ...
    setEditFormData({
      ...
      notes: item.notes || '',
      image: null, // do not pre-fill file input, user must re-upload to change
    });
    setFormErrors({});
    setShowEditModal(true);
    setImagePreviewEdit(
      item.image 
        ? (item.image.startsWith('http') ? item.image : window.location.origin + '/image/' + item.image) 
        : null
    );
  };
              ...
              ${filteredItems.map(item => `
                <tr>
                  <td>
                    ${
                      item.image
                      ? `<img src="${item.image.startsWith('http') ? item.image : window.location.origin + '/image/' + item.image}" alt="فاتورة" class="invoice-thumb" />`
                      : ''
                    }
                  </td>
  ...
  // Show image preview modal when clicking icon in table/mobile card
  const openImageView = (imagePath: string) => {
    const fullPath = imagePath.startsWith('http') ? imagePath : window.location.origin + '/image/' + imagePath;
    setImageViewURL(fullPath);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Inventory Items - Enhanced Mobile View with Image column */}
        ...
        <div className="admin-header flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className="admin-btn-success text-xs flex items-center gap-1 whitespace-nowrap"
            >
              <FileText size={14} />
              Excel
            </button>
            <button
              ...
              className="admin-btn-danger text-xs flex items-center gap-1 whitespace-nowrap"
            >
              <Download size={14} />
              PDF
            </button>
            ...
            <table className="w-full text-sm table-auto">
              <thead>
                <tr className="border-b border-border text-right">
                  ...
                  <th className="p-3">قيمة الشراء</th>
                ...
                {filteredItems.map((item) => (
                  <tr key={item.id} className="border-b border-border text-right hover:bg-accent">
                    <td className="p-2 text-center">
                      {item.image ? (
                        <button
                          onClick={() => openImageView(item.image)}
                    ...
                    <td className="p-3">{item.purchaseValue} ريال</td>
            ...
            <div key={item.id} className="border border-border rounded-lg p-4 bg-card space-y-3">
              <div className="flex justify-between items-start mb-1">
                <div className="text-right flex-1 min-w-0">
                  <h3 className="font-medium text-base truncate">{item.itemName}</h3>
                  <p className="text-sm text-muted-foreground truncate">رقم الصنف: {item.itemNumber}</p>
                </div>
                <span
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="text-right whitespace-nowrap">
                  ...
                  <span className="font-medium mr-1">{item.availableQty}</span>
                </div>
                <div className="text-right whitespace-nowrap">
                  <span className="text-muted-foreground">الحد الأدنى:</span>{' '}
                  <span className="font-medium mr-1">{item.minQuantity}</span>
                </div>
                <div className="text-right whitespace-nowrap">
                  <span className="text-muted-foreground">مستلم:</span>{' '}
                  <span className="font-medium mr-1">{item.receivedQty}</span>
                </div>
                <div className="text-right whitespace-nowrap">
                  <span className="text-muted-foreground">مصروف:</span>{' '}
                  <span className="font-medium mr-1">{item.issuedQty}</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 justify-end flex-wrap">
                {item.image ? (
                  <button
                    onClick={() => openImageView(item.image)}
                    className="admin-btn-info text-xs flex items-center gap-1 whitespace-nowrap"
                    title="عرض فاتورة الشراء"
                    aria-label="عرض فاتورة الشراء"
                  >
                    <ImageIcon size={12} />
                ...
                <button
                  onClick={() => handleEditClick(item)}
                  className="admin-btn-warning text-xs flex items-center gap-1 whitespace-nowrap"
                >
                  <Edit size={12} />
                  تعديل
                </button>
                <button
                  onClick={() => handleWithdrawClick(item)}
                  className="admin-btn-primary text-xs flex items-center gap-1 whitespace-nowrap"
                >
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-6">
              ...
              <div className="admin-card">
                ...
                <div className="p-4 space-y-4">
                  ...
                  {/* فاتورة الشراء image upload */}
                  <div>
                    <label className="block text-sm font-medium mb-2 text-right">فاتورة الشراء (اختياري)</label>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) =>
                        handleAddInputChange('image', e.target.files && e.target.files[0] ? e.target.files[0] : null)
                      }
                      className={`w-full p-1 border rounded-md text-right text-sm ${
                        formErrors.image ? 'border-red-500' : 'border-input'
                      }`}
                    />
                    {formErrors.image && (
                      <p className="text-red-500 text-xs mt-1 text-right">{formErrors.image}</p>
                    )}
                    {imagePreviewAdd && (
                      <div className="mt-2 text-center">
                        <img
                          ...
                          className="mx-auto max-w-[150px] max-h-[150px] rounded-md object-contain border border-border"
                        />
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="mt-1 text-xs text-danger underline hover:text-danger-dark"
                        >
                          إزالة الصورة
                        </button>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-red-500 text-xs mt-1 text-right">
                        هذا هو المبلغ الإجمالي سيتم تقسيمه على الكمية المستلمة
                      </p>
                ...
                <div className="p-4">
                  ...
                  {selectedItem?.image && (
                    <div className="mt-4 text-center">
                      <button
                        type="button"
                        onClick={() => openImageView(selectedItem.image)}
                ...
                <div key={order.id} className="border border-border rounded-lg p-4 bg-card">
                  <div className="flex justify-between items-start mb-3">
                    <div className="text-right flex-1">
                      ...
                      <p className="text-xs text-muted-foreground truncate">الجهة: {order.beneficiaryFacility || 'غير محدد'}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-right">
                    <div>
                      <span className="text-muted-foreground">الكمية:</span> <span className="font-medium mr-1">{order.withdrawQty}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">التاريخ:</span> <span className="font-medium mr-1">{order.withdrawDate}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">المستلم:</span> <span className="font-medium mr-1">{order.recipientName}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">التواصل:</span> <span className="font-medium mr-1">{order.recipientContact}</span>
                    </div>
                  </div>
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.itemNumber ? 'border-red-500' : 'border-input'}`}
                placeholder="رقم الصنف"
                required
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.itemName ? 'border-red-500' : 'border-input'}`}
                placeholder="اسم الصنف"
                required
              />
          ...
          <div>
            ...
            <input
              ...
              accept="image/*"
              onChange={(e) =>
                handleEditInputChange('image', e.target.files && e.target.files[0] ? e.target.files[0] : null)
              }
              className={`w-full p-1 border rounded-md text-right text-sm ${formErrors.image ? 'border-red-500' : 'border-input'}`}
            />
            {formErrors.image && <p className="text-red-500 text-xs mt-1 text-right">{formErrors.image}</p>}
            {imagePreviewEdit && (
              <div className="mt-2 text-center">
                <img
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.receivedQty ? 'border-red-500' : 'border-input'}`}
                placeholder="0"
                required
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.minQuantity ? 'border-red-500' : 'border-input'}`}
                placeholder="الحد الأدنى"
                required
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.purchaseValue ? 'border-red-500' : 'border-input'}`}
                placeholder="0.00"
                required
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.deliveryDate ? 'border-red-500' : 'border-input'}`}
                required
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 border rounded-md text-right text-sm ${formErrors.supplierName ? 'border-red-500' : 'border-input'}`}
                placeholder="اسم الشركة"
                required
              />
...
{/* Image View Modal */}
{showImageViewModal && imageViewURL && (
<div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
<div className="relative bg-background rounded-lg max-w-full max-h-full overflow-auto p-4">
<button
onClick={() => {
...
<img src={imageViewURL} alt="فاتورة الشراء" className="max-w-[90vw] max-h-[80vh] object-contain rounded-md" />
</div>
</div>
)}
